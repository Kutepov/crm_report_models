<?php
class SalesTotalReportForm extends CFormModel
...
// 		$month = $P->sale->FK_control_month;
	...
	public function getLeadsWeekly()
	{
        ...
        $ch = curl_init('https://iqcrm.net/newcrm/index.php/apiB24/main/leads?user_id=432&dateStart=' . date("Y-m-d", $periods[0]));
    ...
    public function  getMonthlySalesOfKind($condition, $strict = true, $typeManager = '', $domains = false)
    {
    	$DS = $this->getBaseStructureQuery();
    	$DS = $this->filterManagers( $DS);
    ...
    public function getMonthlySalesCity($newAndLongOnly = false, $nodev = false)
    {
        ...
        if ($nodev) {
            $criteria->mergeWith(['join' => 'LEFT JOIN {{managers}} Prf ON t.FK_performer = Prf.ID']);
            $criteria->addCondition('NOT(Prf.FK_department = 6 AND (t.FK_sale_product_kind IN (10, 16, 19, 30, 67) OR elaboration.ID = 185))');
        }
    	...
    	foreach ( $iterator as $product )
    	{
    		...
    		// не первый
    		if( $newAndLongOnly && !$this->isFirstSaleProduct($product)) continue;
    		...
    		if(( $M < $this->mEnd - 1) || in_array( $product->FK_sale, $this->whiteList))
    		{
	    		...
	    		$C = $product->sale->company->FK_town;
...
//	    		$MD[ $C ][ $M ] += $product->calcMD(1);
    ...
    public function getMonthlyStructure_LongShort( $value, $strict = true, $nodev = false)
    {
        ...
        foreach ( $iterator as $product )
        {
            ...
            if ($product->elaboration->is_long && !$product->is_unique) $temp = &$long[$M];
            else $temp = &$short[$M];
        ...
        if (Yii::app()->user->userIsROP) {
            ...
            $result = [
                ...
                [
                    ...
                    'items' => [
                        'rop_sales_long' => ['class' => 'col-md-4', 'data' => 1],
                        'rop_sales_short' => ['class' => 'col-md-4', 'data' => 1],
                        'op_quality__client_num' => ['class' => 'col-md-4', 'data' => 1],
                    ]
                ],
                [
                    'header' => '',
            ...
            //Для Юлии Александровой переформируем массив
        } elseif (Yii::app()->request->getParam('asUser') == 338) {
            $result = [
                [
...
//         $result['meets'] = $this->getMeets();
    ...
    protected function getLeads()
    {
        ...
        /*$ch = curl_init('https://api.iqcrm.net/?ajax_action=get_deals&dateStart='
            . $this->startDate->format('Y-m-d') . '&dateEnd=' . $this->endDate->format('Y-m-t').'T'.date('H:i:s'));*/

        $ch = curl_init('https://iqcrm.net/newcrm/index.php/apiB24/main/deals?user_id=432&dateStart='.$this->startDate->format('Y-m-d').'&dateEnd='. $this->endDate->format('Y-m-t').'T'.date('H:i:s'));
    ...
    protected function getPayment()
    {
        ...
        $payment = $this->getQueryPayment($this->startDate->format('Y-m-d'), $this->endDate->format('Y-m-t'));
        $summ1 = 0;
        $summ2 = 0;
        $result = array(
            'month' => array(),
