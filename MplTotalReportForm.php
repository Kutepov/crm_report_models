...
/**
 * Class MplTotalReportForm
 */
class MplTotalReportForm extends CFormModel
...
// 		$month = $P->sale->FK_control_month;
    ...
    private function getBaseStructureQuery($start = 0, $end = 0)
    {
        ...
        $DS->where('S.FK_control_month >= ' . $start)
            ...
            ->andWhere('E.ID IN (17, 26, 223, 213, 243, 245, 76, 193, 33, 64, 197, 230, 153, 144)');
     ...
     * @return CDbCommand
     */
    private function filterMediaPlaners($DS, $all = true)
    {
        $mpls = Yii::app()->accDispatcher->getTriggerGroupList(CrmAccessDispatcher::GROUP_MPLANER);
        if (!empty($this->managers)) {
            $mpls = array_intersect($mpls, $this->managers);
        }

        if (!$all) {
            if (($key = array_search(155, $mpls)) !== false) {
                unset($mpls[$key]);
            }

            foreach ($mpls as $key => $mgrId) {
                $user = User::model()->findByPk($mgrId);
                if (!$user->active) {
                    unset($mpls[$key]);
                }
            }
        }
        return $DS->andWhere('Prf.ID IN (' . implode(', ', $mpls) . ')');
    }
    ...
    public function setDefaults()
    {
        ...
        if (!is_array($this->managers) || count($this->managers) == 0) {
            $this->managers_backup = array_keys($this->getParameterList('mediaplaners'));
        }
     ...
     * @return array
     */
    public function getParameterList($p, $all = true)
    {
        switch ($p) {

            case 'mediaplaners':
                $mplList = Yii::app()->accDispatcher->getTriggerGroupList(CrmAccessDispatcher::GROUP_MPLANER);

                if (!$all) {
                    if (($key = array_search(155, $mplList)) !== false) {
                        unset($mplList[$key]);
                    }

                    foreach ($mplList as $key => $mgrId) {
                        $user = User::model()->findByPk($mgrId);
                        if (!$user->active) {
                            unset($mplList[$key]);
                        }
                ...
                foreach ($mplList as $mpl) {
                    $fios[$mpl] = User::getOneManagerFio($mpl);
                }
                natsort($fios);
                return $fios;
                break;
    ...
    public function performReport()
    {
        ...
        $result = array(
            ...
            [
                ...
                'items' => [
                    'mpl_month_stats' => [ 'class'=>'col-md-4', 'data'=>1],
                    'mpl_premonth_stats' => [ 'class'=>'col-md-4', 'data'=>1],
                ]
            ...
            [
                ...
                'items' => [
                    'mpl_sales_individual' => [ 'class'=>'col-md-12', 'data'=>1],
                ]
     ...
     * @throws Exception
     */
    public function getMonthlySalesOfKindF($firsts = true, $mpl = false)
    {
        $DS = $this->getBaseStructureQuery();
     ...
     * @throws Exception
     */
    public function getMonthlySalesOfMPL()
    {
        $return = [];
        $mpls = Yii::app()->accDispatcher->getTriggerGroupList(CrmAccessDispatcher::GROUP_MPLANER);
        ...
        $return['mpl'] = $this->getParameterList('mediaplaners');

        $return['data'] = [];

        foreach ($mpls as $mpl) {
            $return['data'][$mpl] = [
                'new' => $this->getMonthlySalesOfKindF(true, $mpl),
                'old' => $this->getMonthlySalesOfKindF(false, $mpl),
                'dx' => $this->getMonthlySalesOfKind(false, $mpl),
            ];
        }
     ...
     * @return array
     */
    public function getMonthlySalesOfKind($strict = true, $mpl = false)
    {
        $DS = $this->getBaseStructureQuery();
     ...
     * @throws CException
     */
    public function getMplMonthStructure($value, $strict = true, $first = false, $month = false)
    {
        ...
        $DS = $this->filterMediaPlaners($DS, false);
    ...
    public function getAverageDataByManagers() {
        ...
        $return = [];
        $ms = Yii::app()->accDispatcher->getTriggerGroupList(CrmAccessDispatcher::GROUP_MPLANER);
        //Убираем Иру из отчета
        if (($key = array_search(155, $ms)) !== false) {
            unset($ms[$key]);
        }

        foreach ($ms as $key => $mgrId) {
            $user = User::model()->findByPk($mgrId);
            if (!$user->active) {
                unset($ms[$key]);
            }
